...
def test_restore_media_rechecks_pixel_limit_at_service_boundary():
    from PIL import Image

    inst = _make_instance()
    inst._max_media_items = 1
    inst._max_media_total_bytes = 1024
    inst._max_pixels_per_item = 1
    output = BytesIO()
    Image.new("RGB", (2, 1)).save(output, format="PNG")
    raw = output.getvalue()
    media_id = f"sha256:{hashlib.sha256(raw).hexdigest()}"
    encoded = base64.b64encode(raw).decode("ascii")
    request = SimpleNamespace(
    ...
    with pytest.raises(ValueError, match="service limit=1"):

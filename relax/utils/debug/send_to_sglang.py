...
from relax.utils.data.data import read_file
...
def main(
    ...
    """Minimally send prompts to SGLang using OpenAI endpoints with arguments
    in the same format as main Relax.

    Example usage: python -m relax.utils.debug.send_to_sglang --prompt-data
    /root/datasets/aime-2024/aime-2024.jsonl --input-key prompt --n-samples-
    per-prompt 16 --rollout-max-response-len 32768 --rollout-temperature 1
    --rollout-top-p 1
    """

    async def _main_async():
        df = read_file(prompt_data)
        tasks = [
            asyncio.create_task(_run_one(row.to_dict(), row_index=row_index, repeat_index=repeat_index))
